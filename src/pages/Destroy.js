...
//burn
 ...
 const [startDate, startDateChange] = useState(new Date("2021-07-01 00:00"));
 const [endDate, endDateChange] = useState(new Date("2021-07-01 00:00"));
  const onAuctionPressed = async () => {
	const {success,status} = await auctionNFT(walletAddress,tokId,price,startDate.getTime()/1000,endDate.getTime()/1000);
	setStatus(status);
	setAuctionOpen(false);
  };
 ...
 return (
   <React.Fragment>
    ...
    <div className="Fetcher">
      ...
      <Button variant="contained" color="primary" id="fetchTockens" onClick={::fetchTokens}>获取我的contracts</Button>
      <br></br>
