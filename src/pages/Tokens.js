...
import { connectWallet,fetchNFT, transNFT, listNFT, fetchOneNFT, fetchAllNFT, buyNFT, fetchSellStatus} from "../utils/interact.js";
...
useEffect(async () => {
  ...
  const [listSellOpen, setListSellOpen] = useState(false);
  ...
  const openListDialog = async (e) => {
      ...
      setListDesc(listToken['description']);
       ...
       saleStatus = tradesStatus['tradeStatus'];
  ...
  const handleListSellClose =() => {
            setListSellOpen(false);
  };
...
const onBidPressed = async () => {
        const {buyResult} = await buyNFT(walletAddress,tokId,price);
        setStatus(buyResult);
        setListSellOpen(false);
            ...
            />
             ...
             <input
              ...
              />
               ...
               <DialogActions>
                <Button onClick={handleListSellClose} color="primary">
                 取消
                </Button>
