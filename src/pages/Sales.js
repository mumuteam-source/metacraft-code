...
import { connectWallet,fetchNFT, transNFT, listNFT, fetchOneNFT, salesNFT, buyNFT, fetchSellStatus, cancelSellNFT } from "../utils/interact.js";
...
useEffect(async () => {
  ...
  const fetchSalings = async () => { //TODO: implement
      const {tokuri,tokenIds} = await salesNFT();
      console.log("Counting:",tokuri,tokenIds);
      let cards=[];
  ...
  const openListDialog = async (e) => {
      ...
      const tradeStatus = await fetchSellStatus(tokenId);
      const saleStatus = tradeStatus['tradeStatus'];
  ...
  const onBidPressed = async () => {
	const {buyResult} = await buyNFT(walletAddress,poster,tokId,price);
	setStatus(buyResult);
	setListOpen(false);
  };

  const onCancelPressed = async () => {
	const {cancelResult} = await cancelSellNFT(walletAddress,tokId);
	setStatus(cancelResult);
	setListOpen(false);
 };
  return (
   <React.Fragment>
    ...
    <div className="Fetcher">
      ...
      <Button variant="contained" color="primary" id="fetchTockens" onClick={fetchSalings}>获取在售NFTs</Button>
      <br></br>
      <h1 id="title">在售NFTs</h1>
      <p>
       当前账号:  {walletAddress}
      </p>
	  ...
	  <CardMedia
	   ...
	   />
	    ...
	    />
         ...
         <Button onClick={onCancelPressed} color="primary">
          撤单
         </Button>
      </DialogActions>
    </Dialog>
